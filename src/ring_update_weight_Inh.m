function [Yt, params] = ring_update_weight_Inh(X,total_iter,params,condition_flag)
...
num_samp = size(X,2);
    ...
    %         y = MantHelper.quadprogamYfixed(x,params);
        ...
        if condition_flag % manipulation? 
            params.M = params.M + 5e-6*abs(randn(1)); % add white noise to lateral synapses
        end
    end
end