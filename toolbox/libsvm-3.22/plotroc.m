function [auc,stack_y] = plotroc(y,x,params)
...
% 	plotroc(y,x,model);
	...
	elseif exist('params') && ~ischar(params)
		...
		if fold <= 1
			...
			[auc,stack_y] = roc_curve(deci,label_y); % plot ROC curve
		end
	end
end
...
function [auc,stack_y_interp] = roc_curve(deci,label_y)
    ...
    stack_x_unique = unique(stack_x);
    stack_y_unique = zeros(size(stack_x_unique));
    for xx = 1:length(stack_x_unique)
        tmpid = find(stack_x == stack_x_unique(xx));
        stack_y_unique(xx) = nanmean(stack_y(tmpid));
    end
	plot(stack_x_unique,stack_y_unique);
    stack_y_interp = interp1(stack_x_unique,stack_y_unique,0:0.05:1)';
