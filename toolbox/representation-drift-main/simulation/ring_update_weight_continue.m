function [Yt, params] = ring_update_weight_continue(X,total_iter,params,input_flag)
...
rand_seed = RandStream('mt19937ar','Seed',100);%seeds for reproducibility 
...
num_samp = size(X,2);
    ...
    for i = 1:total_iter
        disp(['Iter = ',num2str(i)])
        if Flag
            y0 = 0.1*rand(rand_seed,params.dim_out,params.batch_size);
