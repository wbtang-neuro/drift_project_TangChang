...
%% get running periods
% linpos = [behavior.timestamps',behavior.position.linearized'];% linpos
...
%% Run UMAP
spike_umap = run_umap(spike_matrix_animal,'min_dist',0.6,'n_neighbors',40,'metric','cosine','n_components',3, 'template_file','PPP21D8_timedecode2.mat');
%% Plot the manifold example, time labels
spike_matrix_label = linpos_RUN(validid,2);% use trial numbers as labels
aa = find(spike_matrix_label~=1 & spike_matrix_label~=2 & spike_matrix_label~=39 & spike_matrix_label~=6);% remove two trials with large noise
...
aa = find(spike_matrix_label <= ep1_trnum(end) & ...
    spike_matrix_label~=1 & spike_matrix_label~=2 & spike_matrix_label~=39 & spike_matrix_label~=6);% trials from epoch 1
spike_matrix_label_choice = linpos_RUN(validid,1);  
...
aa = find(spike_matrix_label > ep1_trnum(end) & ...
    spike_matrix_label~=1 & spike_matrix_label~=2 & spike_matrix_label~=39 & spike_matrix_label~=6); % trials from epoch 2
...
% position colormap
map2 = slanCM('PRGn',round(max(spike_matrix_label_choice(aa))));
traj_colormap=[slanCM('vik',round(maxpos));map2(end:-1:1,:)];
colormap(traj_colormap)
%% Plot the centroids of each position bin
nbins = 100; % bin number
maxpos = ceil(max(spike_umap1_label));
minpos = floor(min(spike_umap1_label));
posbin = minpos:(maxpos-minpos)/nbins:maxpos;
[~,ind] = histc(spike_umap1_label,posbin);
spike_umap_avg = nan(nbins, length(spike_umap1(1,:)));
for i = 1:nbins
    currentid = find(ind == i);
    temp = nanmedian(spike_umap1(currentid,:));
...
nbins = 200;
maxpos = ceil(max(spike_umap2_label));
minpos = floor(min(spike_umap2_label));
posbin = minpos:(maxpos-minpos)/nbins:maxpos;
[~,ind] = histc(spike_umap2_label,posbin);
spike_umap_avg = nan(nbins, length(spike_umap2(1,:)));
for i = 1:nbins
    currentid = find(ind == i);
    temp = nanmedian(spike_umap2(currentid,:));
    spike_umap_avg(i,:) = temp;
end
% remove NaN
temp = sum(spike_umap_avg,2);
...
spike_umap_avg = spike_umap_avg(nonNaN,:);
spike_umap_label = posbin(nonNaN+1);
scatter3(-15*ones(size(spike_umap_avg(:,1))),spike_umap_avg(:,2),spike_umap_avg(:,3),35*ones(size(spike_umap_label)),spike_umap_label,'filled')
zlim([-10,10])
ylim([-8,8])
xlim([-15,15])
view([-162,30])
